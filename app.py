...
def down_video(link, resolution, formatted_time, task_id, upload_folder):
    ...
    yt = YouTube(link, on_progress_callback=lambda stream, chunk, bytes_remaining: video_progress_callback(stream, chunk, bytes_remaining, task_id))
...
def down_audio(link, formatted_time, task_id, upload_folder):
    global progress_status_mp4
    yt = YouTube(link, on_progress_callback=lambda stream, chunk, bytes_remaining: audio_progress_callback(stream, chunk, bytes_remaining, task_id))
...
def down_video_webm(link, resolution, formatted_time, task_id, upload_folder):
    ...
    yt = YouTube(link, on_progress_callback=lambda stream, chunk, bytes_remaining: video_progress_callback_webm(stream, chunk, bytes_remaining, task_id))
...
def down_audio_webm(link, formatted_time, task_id, upload_folder):
    global progress_status_webm
    yt = YouTube(link, on_progress_callback=lambda stream, chunk, bytes_remaining: audio_progress_callback_webm(stream, chunk, bytes_remaining, task_id))
...
def playlist():
    ...
    try:
        ...
        thumbnail_url = YouTube(pl.video_urls[0]).thumbnail_url
